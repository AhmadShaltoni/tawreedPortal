...
import { Plus, Edit, Trash2, Eye, PackageX } from 'lucide-react'
...
export default async function BrandsPage() {
  ...
  const unassignedCount = await db.product.count({ where: { brandId: null } })
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Unassigned products banner */}
      <Link
        href="/admin/brands/unassigned"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-medium text-gray-900">منتجات بدون ماركة</p>
            <p className="text-xs text-gray-600">منتجات غير مصنّفة تحت أي ماركة — اضغط لتصنيفها</p>
          </div>
        </div>
        <span className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-amber-200 text-amber-800 text-sm font-semibold">
          {unassignedCount} منتج
        </span>
      </Link>
