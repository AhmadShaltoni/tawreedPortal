...
import { awardOrderPoints, reverseOrderPoints } from '@/actions/loyalty-points'
import type { ActionResponse } from '@/types'

/**
 * Core resolution of a buyer's order-edit request, shared by the dashboard
 * server actions and the mobile admin API. Authorization is the caller's job;
 * `resolvedById` records which staff user resolved the request.
 *
 * approveOrderEditCore re-prices the proposed items authoritatively (prices/stock
 ...
 * order PENDING. rejectOrderEditCore just marks the request rejected.
 */
...
export async function approveOrderEditCore(
...
export async function rejectOrderEditCore(
  editRequestId: string,
  resolvedById: string,
  adminNote?: string
): Promise<ActionResponse<{ orderId: string }>> {
  ...
  return { success: true, data: { orderId: editRequest.order.id } }
}
