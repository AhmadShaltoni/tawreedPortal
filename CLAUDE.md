...
| **Discount Codes v2 (2026-07-14)** | DiscountCode gains: `maxDiscountCap` (JOD cap on % discounts), `maxUsagePerUser` (N uses per user; supersedes `isSingleUse`, kept in sync when = 1), `firstOrderOnly`, `allowStacking` (combine with loyalty coupons), `allowedUserId` (user-restricted codes; others see NOT_FOUND), and category/product scoping (m2m; % applies to eligible cart lines only). Shared validation extracted to `lib/discount-codes.ts` (used by validate API + orders API); usage recorded race-free via `FOR UPDATE` lock inside the order transaction. `/api/v1/coupons/validate` now computes totals server-side from the cart, accepts `hasLoyaltyCoupon`, returns `allowStacking`. Dead `/api/v1/coupons/confirm` removed. Admin form unified in `app/admin/coupons/CouponForm.tsx` with user/product search pickers. Migration: `20260714000000_discount_code_v2` (additive, **not yet applied to prod**) |
